...
def load_flow_model(name: str, device: str = "cpu", hf_download: bool = True):
...
def load_t5(device: str = "cpu", max_length: int = 512) -> HFEmbedder:
    ...
    model = HFEmbedder("google/t5-v1_1-xxl", max_length=max_length, torch_dtype=torch.bfloat16).to(device)
    return model

def load_clip(device: str = "cpu") -> HFEmbedder:
    model = HFEmbedder("openai/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(device)
    return model

def load_ae(name: str, device: str = "cpu", hf_download: bool = True) -> AutoEncoder:
