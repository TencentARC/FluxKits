...
def get_models(name: str, device, offload: bool, is_schnell: bool, args):
    ...
    model = load_flow_model2(name, args, device="cpu")
...
@torch.no_grad()
def inference_prompts(prompts, dit, t5, clip, vae, is_schnell,
        height=512, width=512, device='npu:0', dtype=torch.float, seed=123):
    ...
    for i, prompt in enumerate(prompts):
        ...
        denoised = denoise_simple(dit, **input, timesteps=timesteps, guidance=3.5)
...
def main():
    ...
    dit, vae, t5, clip = get_models(name=args.model_name, device=accelerator.device, offload=False, is_schnell=is_schnell, args=args)
    lora_attn_procs = {}
    ...
    params = [p for n, p in dit.named_parameters() if '_lora' in n]
    print(sum([p.numel() for p in params if p.requires_grad]) / 1000000, 'M parameters')
    ...
    num_update_steps_per_epoch = 10000 # math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(dit):
                img, prompts = batch
                with torch.no_grad():
                    x_1 = vae.encode(img.to(f'npu:{accelerator.device}' if isinstance(accelerator.device, int) else accelerator.device).to(torch.float32))
                    inp = prepare(t5=t5, clip=clip, img=x_1, prompt=prompts)
                    x_1 = rearrange(x_1, "b c (h ph) (w pw) -> b (h w) (c ph pw)", ph=2, pw=2)

                bs = img.shape[0]
                t = torch.sigmoid(torch.randn((bs,), device=f'npu:{accelerator.device}' if isinstance(accelerator.device, int) else accelerator.device))

                x_0 = torch.randn_like(x_1).to(f'npu:{accelerator.device}' if isinstance(accelerator.device, int) else accelerator.device)
                x_t = (1 - t) * x_1 + t * x_0
                bsz = x_1.shape[0]
                guidance_vec = torch.full((x_t.shape[0],), 4, device=f'npu:{x_t.device}' if isinstance(x_t.device, int) else x_t.device, dtype=x_t.dtype)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.every_log_image == 0:
                    if accelerator.is_main_process:
                        dit.eval()
                        images = inference_prompts(prompts, dit, t5, clip, vae, is_schnell)
                        for i, image in enumerate(images):
                            image = Image.fromarray((127.5 * (image + 1.0)).cpu().byte().numpy())
                            save_path = os.path.join(img_dir, f'{global_step}-{i}.jpg')
