...
def load_checkpoint(local_path, repo_id, name):
    if local_path is not None:
        if '.safetensors' in local_path:
            logger.info(f"Loading .safetensors checkpoint from {local_path}")
            checkpoint = load_safetensors(local_path)
        else:
            logger.info(f"Loading checkpoint from {local_path}")
            checkpoint = torch.load(local_path, map_location='cpu')
    elif repo_id is not None and name is not None:
        logger.info(f"Loading checkpoint {name} from repo id {repo_id}")
...
def print_load_warning(missing: list[str], unexpected: list[str]):
...
def load_flow_model(name: str, device: str = "npu", hf_download: bool = True):
...
def load_flow_model2(name: str, args, device: str = "cuda", hf_download: bool = True):
    ckpt_path = configs[name].ckpt_path
    if (
        ckpt_path is None
        and configs[name].repo_id is not None
...
# choose the weights for initializing the student model
def pick_up_weights(weights, args):
    source_idx_double = args.distill_slim_double
    source_idx_single = args.distill_slim_single
...
def load_flow_model_stu(name: str, args=None, 
                     device: str = "npu",
                     init_shift = False):
    with torch.device(device):
        params = deepcopy(configs[name].params)
        
        params.depth = len(args.distill_slim_double)
        params.depth_single_blocks = len(args.distill_slim_single)
        params.args = args
...
def load_t5(device: str = "npu", max_length: int = 512) -> HFEmbedder:
    # max length 64, 128, 256 and 512 should work (if your sequence is short enough)
    model = HFEmbedder("google/t5-v1_1-xxl", max_length=max_length, torch_dtype=torch.bfloat16).to(f'npu:{device}' if isinstance(device, int) else device)
    return model

def load_clip(device: str = "npu") -> HFEmbedder:
    model = HFEmbedder("openai/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(f'npu:{device}' if isinstance(device, int) else device)
    return model

def load_ae(name: str, device: str = "npu", hf_download: bool = True) -> AutoEncoder:
...
class WatermarkEmbedder:
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        ...
        image = torch.from_numpy(rearrange(image_np[:, :, :, ::-1], "(n b) h w c -> n b c h w", n=n)).to(
            f'npu:{image.device}' if isinstance(image.device, int) else image.device)
