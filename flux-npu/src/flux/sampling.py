...
def denoise(
    ...
    ip_scale: Tensor = 1.0,
    neg_ip_scale: Tensor = 1.0
):
    i = 0
    # this is ignored for schnell
    guidance_vec = torch.full((img.shape[0],), guidance, device=f'npu:{img.device}' if isinstance(img.device, int) else img.device, dtype=img.dtype)
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=f'npu:{img.device}' if isinstance(img.device, int) else img.device)
        pred = model(
            img=img,
            img_ids=img_ids,
            txt=txt,
