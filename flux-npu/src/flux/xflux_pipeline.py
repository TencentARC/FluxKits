...
class XFluxPipeline:
    ...
    def set_ip(self, local_path: str = None, repo_id = None, name: str = None):
        self.model.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device)
        ...
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained(self.image_encoder_path).to(
            f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=torch.float16
        )
        ...
        self.improj = self.improj.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=torch.bfloat16)
        ...
        for name, _ in self.model.attn_processors.items():
            ...
            if ip_state_dict:
                ...
                ip_attn_procs[name].to(f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=torch.bfloat16)
            else:
    ...
    def update_model_with_lora(self, checkpoint, lora_weight):
        ...
        for name, _ in self.model.attn_processors.items():
            ...
            lora_attn_procs[name].to(f'npu:{self.device}' if isinstance(self.device, int) else self.device)
    ...
    def get_image_proj(
        ...
        image_prompt = image_prompt.to(f'npu:{self.image_encoder.device}' if isinstance(self.image_encoder.device, int) else self.image_encoder.device)
        image_prompt_embeds = self.image_encoder(
            image_prompt
        ).image_embeds.to(
            f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=torch.bfloat16,
        )
        # encode image
    ...
    def __call__(self,
        ...
        if self.controlnet_loaded:
            ...
            controlnet_image = controlnet_image.permute(
                2, 0, 1).unsqueeze(0).to(torch.bfloat16).to(f'npu:{self.device}' if isinstance(self.device, int) else self.device)
    ...
    def forward(
        ...
        with torch.no_grad():
            if self.offload:
                self.t5, self.clip = self.t5.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device), self.clip.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device)
            ...
            if self.offload:
                ...
                self.model = self.model.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device)
            if self.controlnet_loaded:
            ...
            if self.offload:
                ...
                self.ae.decoder.to(f'npu:{x.device}' if isinstance(x.device, int) else x.device)
